...
load(file.path(paste0(outputdt, "/" ,"dtforPSA.rda", sep ="")))

PSAdt_group <- list()
bestfit <- list()
for(i in names(dtforPSA)){ 
  dtforPSA[[i]] <- dtforPSA[[i]]%>%
    dplyr::select(`QALY gained`, `Incremental Cost`,population, testing, simulation)
  
  bestfit[[i]] <- dtforPSA[[i]]%>%filter(simulation=="best")%>%
    group_split(testing)
  
  PSAdt_group[[i]] <- dtforPSA[[i]]%>%filter(simulation!="best")%>%
    group_split(testing)
  
  names(PSAdt_group[[i]]) <- levels(dtforPSA[[i]]$testing)
  names(bestfit[[i]]) <- levels(dtforPSA[[i]]$testing)
  }
...
contour.levels <- c(0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1)
...
WTP.thresholds <- c(32811, 32811*3)
...
basecase <- bestfit$PrEPsame$`Point-of-care anibody testing`%>%
                           filter(population == popSelect[1])
basecase <- as.numeric(basecase[1,c(1,2)])
# 
...
# Load data
popSelect <- unique(PSAdt_group$PrEPsame$`Point-of-care anibody testing`$population)
data <- PSAdt_group$PrEPsame$`Point-of-care anibody testing`%>%filter(population == popSelect[1])
