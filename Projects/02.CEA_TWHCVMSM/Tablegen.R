...
x <- dt_tab_char %>% 
  ...
  mutate(Med = case_when(indicator %in% c("Lifetime cost (discounted, millions US$)")~ round(Med/1000000, digits = 1),
                          indicator %in% c("Lifetime QALY (discounted, thousands)")~ round(Med/1000, digits = 0),
         ...
         q5= case_when(indicator %in% c("Lifetime cost (discounted, millions US$)")~ round(q5/1000000, digits = 1),
                          indicator %in% c("Lifetime QALY (discounted, thousands)") ~ round(q5/1000, digits = 0),
                       indicator %in% c("New HCV infections (2022-2030)",
                                        "New HCV primary infections (2022-2030)",
         ...
         q95 = case_when(indicator %in% c("Lifetime cost (discounted, millions US$)")~ round(q95/1000000, digits = 1),
                          indicator %in% c("Lifetime QALY (discounted, thousands)")~ round(q95/1000,digits = 0),
                         indicator %in% c("New HCV infections (2022-2030)",
                                          "New HCV primary infections (2022-2030)",
...
# combine stage cost as separate indicators 
 ...
 dt_incre <-  dt_incre%>%mutate(testing = factor(testing, 
        ...
        indicator = factor(indicator, 
                           ...
                           label = c("HCV infections averted (2022-2030)",
                                     ...
                                     "Incremental lifetime cost (discounted, millions US$)",
                                     "Incremental lifetime cost of HCV diagnosis (discounted, millions US$)",
                                     "Incremental lifetime cost of HCV treatment (discounted, millions US$)",
                                     "Incremental lifetime cost of HCV management (discounted, millions US$)",
 ...
 dt_incre <- dt_incre %>% 
   select(scenario, indicator, population, testing, 
          Med, q5, q95)%>%
  filter(!indicator %in% c("Incremental lifetime cost of HCV diagnosis (discounted, millions US$)",
                           "Incremental lifetime cost of HCV treatment (discounted, millions US$)",
                           "Incremental lifetime cost of HCV management (discounted, millions US$)"))%>%
   mutate(Med = case_when(indicator %in% 
                            c("Incremental lifetime cost (discounted, millions US$)")~ round(Med/1000000, digits = 0),
   ...
   q5= case_when(indicator %in% c("Incremental lifetime cost (discounted, millions US$)")~ round(q5/1000000, digits = 0),
                 indicator %in% c("QALY gained (lifetime)") ~ round(q5, digits = 0),
                 indicator %in% c("HCV infections averted (2022-2030)")~ round(q5, digits =  1),
                 .default = q5
   ),
   q95 = case_when(indicator %in% c("Incremental lifetime cost (discounted, millions US$)")~ round(q95/1000000, digits = 0),
...
# long form 
dt_incre%>%tidyr::pivot_wider(names_from = c("indicator","population"), 
                      values_from = c("vv"), 
                      names_glue = "{indicator}X{population}")%>%
   # grouping rows
   gt(groupname_col = "scenario",
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = c(`ICERXHIV-negative MSM on PrEP`),
       rows = c(as.numeric(`MedXHIV-negative MSM on PrEP`) > 1*TW_GDP2022 & 
                  c(as.numeric(`MedXHIV-negative MSM on PrEP`) <= 3*TW_GDP2022))))%>%
   tab_style(
     style = list(
     ...
     locations = cells_body(
       columns = c(`ICERXHIV-negative MSM on PrEP`),
       rows = c(as.numeric(`MedXHIV-negative MSM on PrEP`) > 3*TW_GDP2022 )
     ))%>%
   tab_style(
     style = list(
       cell_fill(color = "#DEEBF7"),
   ...
   tab_style(
     ...
     locations = cells_body(
       columns = c(`ICERXEntire population of MSM`),
       rows = c(as.numeric(`MedXEntire population of MSM`) > 1*TW_GDP2022 & 
                  c(as.numeric(`MedXEntire population of MSM`) <= 3*TW_GDP2022)
       )))%>%
   tab_style(
     style = list(
       cell_fill(color = "#08519C"),
     ...
     locations = cells_body(
       columns = c(`ICERXEntire population of MSM`),
       rows = c(as.numeric(`MedXEntire population of MSM`) > 3*TW_GDP2022 )
     )
   )%>%
   tab_style(
     style = list(
   ...
   tab_style(
     ...
     locations = cells_body(
       ...
       rows = c(as.numeric(`MedXMSM who are HIV diagnosed (and on treatment)`) > 1*TW_GDP2022 & 
                  c(as.numeric(`MedXMSM who are HIV diagnosed (and on treatment)`) <= 3*TW_GDP2022)
       )
     ))%>%
   tab_style(
     style = list(
     ...
     locations = cells_body(
       columns = c(`ICERXMSM who are HIV diagnosed (and on treatment)`),
       rows = c(as.numeric(`MedXMSM who are HIV diagnosed (and on treatment)`) > 3*TW_GDP2022 )
     )
   )%>%
...
for(i in seq_along(tt$cumu_lifecost_stage)){ 
  ...
  plot_costcasca[[i]] <- lifecost_stage[[i]]%>%
    ...
    labs(fill = "HCV Cascade", tag = plot_tag[i]) + 
