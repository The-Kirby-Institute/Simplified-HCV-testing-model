...
project_name <- "POC_states"
# set up dircetory 
basePath <- paste("/Users/jjwu/Library/CloudStorage/OneDrive-SharedLibraries-UNSW/Joyce & Richard - General/", 
                               project_name, sep = "")
dir.create(file.path(paste(basePath, "/01. DATA", sep = ""))) # create subdircetory
dir.create(file.path(paste(basePath, "/01. DATA/data input", sep = "")))
dir.create(file.path(paste(basePath, "/02. Output", sep = ""))) 
dir.create(file.path(paste(basePath, "/02. Output/RDA", sep = "")))
project_directory <- basePath
DataFolder  <- file.path(basePath, "01. DATA/data input")
...
POC_states <-list()
POC_state$project_name <- project_name


# population
POC_state$npops <- npops
POC_state$popNames <- pop_names

# component
POC_state$component_name <- compon_name
POC_state$ncomponent <- length(compon_name)

# component without s
POC_state$component_nameWOS <- compon_nameWOS
POC_state$ncomponentWOS <- length(compon_nameWOS)

# disease progress
POC_state$progress_name <- short_progress_name[-1]
POC_state$nprogress <- length(short_progress_name)-1

# disease progression rate 
POC_state$diseaseprogress_Name <- transitionName
POC_state$diseaseprogress_n <- length(transitionName)

# disease progression rate that are constant overtime and populations
POC_state$fibName <- fibName
POC_state$fib_n <-length(fibName)

# cascade
POC_state$cascade_name <- short_cascade_name
POC_state$ncascade <- length(short_cascade_name)
# time related
POC_state$startYear <- startYear
POC_state$endYear <- endYear
