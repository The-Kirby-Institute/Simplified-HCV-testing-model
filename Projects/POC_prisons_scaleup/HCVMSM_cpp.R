HCVMSM_cpp <- function(HCV, parama, initialPop, disease_progress,
  ...
  # ── Same preprocessing as original HCV_np ───────────────────────────────
  if (is.null(end_Y)) {
    npts <- HCV$npts - 1
  ...
  # SVR adjustments (match HCV_np exactly)
  ...
  # ── pop_array dt transform (FIXED to match HCV_np) ─────────────────────
  # Previously: pop_array_dt <- pop_array * dt   (linear approx — wrong)
  # HCV_np uses: pop_array <- 1-(1-pop_array)^dt
  pop_array_dt <- 1 - (1 - pop_array)^dt
  
  # ── FOI matrix ─────────────────────────────────────────────────────────
  # HCV_np: foi_dt <- foi * dt   (linear, no compounding)
