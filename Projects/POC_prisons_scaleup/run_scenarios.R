...
project_name <- "POC_prisons_scaleup"
...
output_path <- paste(codefun_path, "/Projects/", project_name, sep = "")
...
load(file.path(RDAFolder, paste0("POC_AU", ".rda")))
load(file.path(RDAFolder, paste0("POC_AU", "cali.rda")))
load(file.path(RDAFolder, paste0("POC_AU", "cali_timev.rda")))
load(file.path(RDAFolder, paste0("POC_AU", "_NPcal_2024.rda")))
...
Rcpp::sourceCpp(file.path(codefun_path, "/Projects/", project_name, "/hcvmsm_core.cpp"))
source(file.path(codefun_path, "/Projects/", project_name,"/HCVMSM_cpp.R"))
...
#   (A) make_test_scenario(totals, comm_frac): legacy, single split fraction.
#   (B) make_test_scenario_explicit(comm_vec, prison_vec): NEW — per-year
#       explicit community and prison totals (no derived split fraction).
#
...
# Legacy helper (kept so `no_np` still works the same way)
...
# New helper: explicit per-year community and prison targets
make_test_scenario_explicit <- function(comm_vec, prison_vec) {
  stopifnot(length(comm_vec)   == length(scenario_years))
  stopifnot(length(prison_vec) == length(scenario_years))
  tibble(
    year       = scenario_years,
    T_total    = comm_vec + prison_vec,
    T_target_C = comm_vec,
    T_target_P = prison_vec
  )
}

scenarios <- list(
  # Status quo: no NP from 2025 onward
  no_np = make_test_scenario(rep(0, 6)),
  
  # Scenario 1 — sustained, equal split (no overall increase)
  sustained_equal_split = make_test_scenario_explicit(
    comm_vec   = c(11000, 11000, 11000, 11000, 11000, 11000),
    prison_vec = c(14000, 14000, 14000, 14000, 14000, 14000)
  ),
  
  # Scenario 2 — sustained total, community→prison shift
  sustained_prison_shift = make_test_scenario_explicit(
    comm_vec   = c(11000, 10000,  9000,  8000,  7000,  6000),
    prison_vec = c(14000, 15000, 16000, 17000, 18000, 19000)
  ),
  
  # Scenario 3 — scale-up, equal split
  scaleup_equal_split = make_test_scenario_explicit(
    comm_vec   = c(11000, 11000, 13750, 16500, 19250, 22000),
    prison_vec = c(14000, 14000, 17500, 21000, 24500, 28000)
  ),
  
  # Scenario 4 — scale-up, prison focus
  scaleup_prison_focus = make_test_scenario_explicit(
    comm_vec   = c(11000, 11000, 12000, 12000, 12000, 12000),
    prison_vec = c(14000, 14000, 19250, 25500, 31750, 38000)
  ),
  
  # Scenario 5 — scale-up, community focus
  scaleup_community_focus = make_test_scenario_explicit(
    comm_vec   = c(11000, 11000, 18250, 24500, 30750, 37000),
    prison_vec = c(14000, 14000, 13000, 13000, 13000, 13000)
  )
)

# Quick sanity print of scenario totals
cat("\nScenario target totals (year x setting):\n")
for (snm in names(scenarios)) {
  cat(sprintf("\n  %s\n", snm))
  print(scenarios[[snm]])
}
...
# =============================================================================
# AUTO-FIT WRAPPER: Find Cov_np, Cov_neg to hit T_target within tolerance
...
#   Phase 2: If Phase 1 can't hit tolerance, allow fc to drift by searching
#            Cov_neg independently while Cov_np stays at the Phase 1 best m.
#   Max iterations per year per setting (combined): max_iter_total.
...
run_single_year_manual <- function(year = 2025,
  ...
  # ---- Run scenario (C++ engine, epi only) ----
  Sce <- HCVMSM_cpp(
...
# --- Step 2: run final simulations (epi only, C++ engine) ---
cat(sprintf("\n=============================================================================\n"))
cat(sprintf("Running final simulations (HCVMSM_cpp, epi only)\n"))
cat(sprintf("=============================================================================\n"))
...
toc <- proc.time() - tic
cat(sprintf("\nFinal simulations total time: %.1f sec\n", toc[3]))

# Save (epi only)
save(
  Sce_np,                  # final simulations per scenario (epi only)
  scenario_cascade,        # NP cascade per scenario
  ...
  file = file.path(OutputFolder,
                   paste0(project_name, "Simulations_epi.rda"))
)

cat(sprintf("Saved: %sSimulations_epi.rda\n", project_name))
...
# Load back the epi simulation file for validation
load(file.path(OutputFolder, paste0(project_name, "Simulations_epi.rda")))
