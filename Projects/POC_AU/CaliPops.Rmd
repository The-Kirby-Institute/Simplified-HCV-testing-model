...
datapoint[["abPrev"]] <- 
  cbind.data.frame(year = rep(POC_AU$simY, length(POC_AU$popNames) + 1), 
                   ...
                   realpop = c(NA, NA, 91.1, NA, NA, 27.8),
...
tempPrev_subpop <- tempPrev_subpop%>%
  mutate(population = factor(population, 
                             ...
                             labels = pop_labname ))
popPrevPlot <- list()
for(i in 1: length(pop_labname)){ 
  popPrevPlot[[i]] <- indicatorPlot(POC_AU, tempPrev_subpop%>%
                                      filter(population ==c(unique(tempPrev_subpop$population))[i]), 
    ...
    ggtitle(paste0("HCV prevalence:", pop_labname[i])) + 
    scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, 10))
    
  }
popPrevPlot[[3]] <- popPrevPlot[[3]] + 
  geom_point(aes(
    y=datapoint[["abPrev"]][datapoint[["abPrev"]][, "indicator"] == "P_PWID", "realpop"], 
    x = POC_AU$simY ), colour = "black")
...
##Figure10. HCV prevalence  


```{r echo=FALSE, warning=FALSE, message=FALSE,  fig.height= 9, fig.width = 12, fig.cap = "" }
ggpubr::ggarrange(popPrevPlot[[3]]) + 
  theme(plot.margin = margin(0.1,0.1,0.1,0.1, "cm")) 
