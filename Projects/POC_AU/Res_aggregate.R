...
for(i in names(Resflow_dt)){ 
  Resflow_year_pop[[i]] <- Resflow_dt[[i]]%>%as_tibble()%>%arrange(population, year)%>%
    ...
    mutate(newInf_year = cumsum(newInfections), 
           ...
           Treatment_sc_year = cumsum(Treatment_sc), 
    ...
    mutate(newInf_ycum = ifelse(index == 11, newInf_year, NA), 
           ...
           Treatment_sc_ycum = ifelse(index == 11, Treatment_sc_year, NA),
    ...
    select(year, timestep, population, newInfections, newInf_ycum, 
           ...
           Treatment_sc, Treatment_sc_ycum,
           Testing_ab_sc, Testing_ab_sc_ycum, Testing_RNA_sc, Testing_RNA_sc_ycum, 
           Testing_POCT_sc, Testing_POCT_sc_ycum,
           Testing_ab_sc_neg, Testing_ab_sc_neg_ycum, Testing_RNA_sc_neg, Testing_RNA_sc_neg_ycum, 
...
for(i in names(Resflow_dt)){ 
  Resflow_year_all[[i]] <- Resflow_year_pop[[i]]%>%ungroup()%>%
    ...
    select(year, timestep, newInfections, HCVdeath, Treatment, Retreat, Cured, 
           ...
           Treatment_sc,
           Testing_ab_sc, Testing_RNA_sc, Testing_POCT_sc,
           Testing_ab_sc_neg, Testing_RNA_sc_neg, Testing_POCT_sc_neg)
}
...
for(i in names(Rescost_dt)){ 
  ...
  RescostDAA[[i]] <- cbind(year = Resflow_year_pop[[i]]$year, 
                           ...
                           newTreat_sc = Resflow_year_pop[[i]]$Treatment_sc,
                           costTreat_sc = Rescost_dt[[i]]$cost_Treatment_sc)%>%
    as_tibble()%>%
    mutate(TreatDAA_cost = newTreat*unitC_DAA,
           RetreatDAA_cost = newRetreat*unitC_secline_DAA,
           TreatDAA_sc_cost = newTreat_sc*unitC_DAA)%>%
    mutate(TreatOther_cost = costTreat - TreatDAA_cost - costTreat_sc, 
           RetreatOther_cost = costRetreat - RetreatDAA_cost,
           TreatOther_sc_cost = costTreat_sc - TreatDAA_sc_cost)%>%
    mutate(totalDAA_cost = TreatDAA_cost + RetreatDAA_cost + TreatDAA_sc_cost)%>%
    ungroup()
...
for(i in names(RescostDAA)){
  ...
  RescostDAA_totalpop[[i]] <- RescostDAA[[i]]%>%ungroup()%>%
    ...
    summarise(
      ...
      newTreat_sc = sum(newTreat_sc, na.rm = TRUE), 
      TreatDAA_cost = sum(TreatDAA_cost, na.rm = TRUE),
      TreatDAA_sc_cost = sum(TreatDAA_sc_cost, na.rm = TRUE),
      TreatOther_cost = sum(TreatOther_cost, na.rm = TRUE), 
      TreatOther_sc_cost = sum(TreatOther_sc_cost, na.rm = TRUE), 
...
rep_col <- c("TreatDAA_cost", "TreatOther_cost", "RetreatDAA_cost", 
             "RetreatOther_cost", "TreatDAA_sc_cost", "TreatOther_sc_cost", "totalDAA_cost")
...
for(i in names(Rescost_dt)){ 
  Rescost_DAA_sc_dt[[i]] <- Rescost_DAA_dt[[i]]%>%
...
colnames(Rescost_DAA_dt$dfList_NP_2023)
...
for(i in names(Rescost_dt)){ 
  Rescost_year[[i]] <- Rescost_DAA_dt[[i]]%>%as_tibble()%>%
    ...
    mutate(cost_total_year = cumsum(cost_total), 
           ...
           cost_ab_sc_year = cumsum(cost_ab_sc), 
           cost_RNA_sc_year = cumsum(cost_RNA_sc), 
           cost_POCT_sc_year = cumsum(cost_POCT_sc), 
           cost_TreatDAA_sc_year = cumsum(TreatDAA_sc_cost), 
           cost_TreatOther_sc_year = cumsum(TreatOther_sc_cost)
           )%>%
    # finding the last row in each year and pop
    mutate(index = c(round((timestep%%1)/POC_AU$timestep, digits = 0)))%>%
    mutate(cost_total_ycum = ifelse(index == 11, cost_total_year, NA), 
           ...
           cost_ab_sc_ycum = ifelse(index == 11, cost_ab_sc_year, NA),
           cost_RNA_sc_ycum = ifelse(index == 11, cost_RNA_sc_year, NA),
           cost_POCT_sc_ycum = ifelse(index == 11, cost_POCT_sc_year, NA),
           cost_TreatDAA_sc_ycum = ifelse(index == 11, cost_TreatDAA_sc_year, NA), 
           cost_TreatOther_sc_ycum = ifelse(index == 11, cost_TreatOther_sc_year, NA)
           
    )%>%
    mutate(cost_TreatDAA = TreatDAA_cost, 
           cost_RetreatDAA = RetreatDAA_cost,
           cost_TreatDAA_sc = TreatDAA_sc_cost, 
           cost_TreatOther = TreatOther_cost, 
           cost_RetreatOther = RetreatOther_cost, 
           cost_TreatOther_sc = TreatOther_sc_cost, 
    ...
    select(year, timestep, population, cost_total, cost_total_ycum, 
           ...
           cost_ab_sc, cost_ab_sc_ycum, 
           cost_RNA_sc, cost_RNA_sc_ycum, cost_POCT_sc, cost_POCT_sc_ycum, 
           cost_TreatDAA_sc, cost_TreatDAA_sc_ycum,
           cost_TreatOther_sc, cost_TreatOther_sc_ycum)
}
...
for(i in names(Rescost_DAA_dt)){ 
  Rescost_sc_year[[i]] <- Rescost_DAA_sc_dt[[i]]%>%as_tibble()%>%
    ...
    mutate(cost_ab_year = cumsum(cost_ab_sc), 
           ...
           cost_TreatDAA_year = cumsum(TreatDAA_sc_cost), 
           
           cost_TreatOther_year = cumsum(TreatOther_sc_cost)
           )%>%
    mutate(index = c(round((timestep%%1)/POC_AU$timestep, digits = 0)))%>%
    ...
    mutate(cost_ab = cost_ab_sc,
           ...
           cost_TreatDAA = TreatDAA_sc_cost, 
          
           cost_TreatOther = TreatOther_sc_cost)%>%
  ...
  Rescost_yearcum_pop[[i]] <- Rescost_year[[i]]%>%as_tibble()%>%arrange(population)%>%
   group_by(population)%>%
    mutate(cost_total_cum = cumsum(cost_total), 
           ...
           cost_ab_sc_cum = cumsum(cost_ab_sc), 
           cost_RNA_sc_cum = cumsum(cost_RNA_sc),
           cost_POCT_sc_cum = cumsum(cost_POCT_sc),
           cost_TreatDAA_sc_cum = cumsum(cost_TreatDAA_sc), 
           cost_TreatOther_sc_cum = cumsum(cost_TreatOther_sc)
           )%>%
    mutate(cost_total_cum = ifelse(is.na(cost_total_ycum), NA, cost_total_cum),
           ...
           cost_ab_sc_cum = ifelse(is.na(cost_ab_sc_ycum), NA, cost_ab_sc_cum),
           cost_RNA_sc_cum = ifelse(is.na(cost_RNA_sc_ycum), NA, cost_RNA_sc_cum),
           cost_POCT_sc_cum = ifelse(is.na(cost_POCT_sc_ycum), NA, cost_POCT_sc_cum),
           cost_TreatDAA_sc_cum = ifelse(is.na(cost_TreatDAA_sc_ycum), NA, cost_TreatDAA_sc_cum),
           cost_TreatOther_sc_cum = ifelse(is.na(cost_TreatOther_sc_ycum), NA, cost_TreatOther_sc_cum),
    ...
    mutate(cost_total_disycum = ifelse(is.na(cost_total_ycum)|is.na(discount), 0, cost_total_ycum/discount), 
           ...
           cost_ab_sc_disycum  = ifelse(is.na(cost_ab_sc_ycum)|is.na(discount), 0, cost_ab_sc_ycum/discount), 
           cost_RNA_sc_disycum  = ifelse(is.na(cost_RNA_sc_ycum)|is.na(discount), 0, cost_RNA_sc_ycum/discount), 
           cost_POCT_sc_disycum  = ifelse(is.na(cost_POCT_sc_ycum)|is.na(discount), 0, cost_POCT_sc_ycum/discount), 
           cost_TreatDAA_sc_disycum  = ifelse(is.na(cost_TreatDAA_sc_ycum)|is.na(discount), 0, cost_TreatDAA_sc_ycum/discount), 
           cost_TreatOther_sc_disycum  = ifelse(is.na(cost_TreatOther_sc_ycum)|is.na(discount), 0, cost_TreatOther_sc_ycum/discount)
    ...
    mutate(cost_total_discum = cumsum(cost_total_disycum), 
           ...
           cost_ab_sc_discum  = cumsum(cost_ab_sc_disycum), 
           cost_RNA_sc_discum  = cumsum(cost_RNA_sc_disycum), 
           cost_POCT_sc_discum  = cumsum(cost_POCT_sc_disycum), 
           cost_TreatDAA_sc_discum  = cumsum(cost_TreatDAA_sc_disycum), 
           cost_TreatOther_sc_discum  = cumsum(cost_TreatOther_sc_disycum)
    )%>%
    select(year, timestep, population,discount, 
           ...
           cost_ab_sc, cost_ab_sc_ycum, cost_ab_sc_cum, cost_ab_sc_disycum, cost_ab_sc_discum, 
           
           cost_RNA_sc, cost_RNA_sc_ycum, cost_RNA_sc_cum, cost_RNA_sc_disycum, 
           cost_RNA_sc_discum, 
           
           cost_POCT_sc, cost_POCT_sc_ycum, cost_POCT_sc_cum, cost_POCT_sc_disycum, 
           cost_POCT_sc_discum, 
           
           cost_TreatDAA_sc, cost_TreatDAA_sc_ycum, cost_TreatDAA_sc_cum, 
           cost_TreatDAA_sc_disycum, cost_TreatDAA_sc_discum, 
           
           cost_TreatOther_sc, cost_TreatOther_sc_ycum, cost_TreatOther_sc_cum, 
           cost_TreatOther_sc_disycum, cost_TreatOther_sc_discum
...
for(i in names(Rescost_dt)){ 
  Rescost_year_all[[i]] <- Rescost_year[[i]]%>%ungroup()%>%group_by(timestep)%>%
    summarise(cost_total = sum(cost_total),
              ...
              cost_ab_sc = sum(cost_ab_sc), 
              cost_RNA_sc = sum(cost_RNA_sc), 
              cost_POCT_sc = sum(cost_POCT_sc), 
              cost_TreatDAA_sc = sum(cost_TreatDAA_sc),
              cost_TreatOther_sc = sum(cost_TreatOther_sc)
    ...
    mutate(cost_total_year = cumsum(cost_total),
           ...
           cost_RNA_sc_year = cumsum(cost_RNA_sc), 
           cost_POCT_sc_year = cumsum(cost_POCT_sc), 
           cost_TreatDAA_sc_year = cumsum(cost_TreatDAA_sc), 
           cost_TreatOther_sc_year = cumsum(cost_TreatOther_sc),
           )%>%
    # finding the last row in each year and pop
    mutate(program = "non-NP",
           index = c(round((timestep%%1)/POC_AU$timestep, digits = 0)))%>%
    mutate(cost_total_ycum = ifelse(index == 11, cost_total_year, NA), 
           cost_compartment_ycum = ifelse(index == 11, cost_compartment_year, NA),
           cost_ab_ycum = ifelse(index == 11, cost_ab_year, NA),
           cost_RNA_ycum = ifelse(index == 11, cost_RNA_year, NA),
...
epi_index <- c("Treatment", "Treatment_ycum", "Retreat", "Retreat_ycum", 
               ...
               "Testing_ab_neg",  "Testing_ab_neg_ycum",      
               "Testing_RNA_neg", "Testing_RNA_neg_ycum",     
               "Testing_POCT_neg", "Testing_POCT_neg_ycum",   
               
               "Testing_ab_sc", "Testing_ab_sc_ycum", 
               "Testing_RNA_sc", "Testing_RNA_sc_ycum", "Testing_POCT_sc", "Testing_POCT_sc_ycum",
               "Treatment_sc", "Treatment_sc_ycum", 
               "Testing_ab_sc_neg", "Testing_ab_sc_neg_ycum",   
               "Testing_RNA_sc_neg", "Testing_RNA_sc_neg_ycum",  
               "Testing_POCT_sc_neg", "Testing_POCT_sc_neg_ycum"
               )
# cost_indicator includes the cost of NP 
# cost_indicator_sc inlcude only cost of NP 
for(i in names(Rescost_yearcum_pop)){ 
  ...
  epicost_pop[[i]] <- cbind(Rescost_yearcum_pop[[i]],
    ...
    select(year, timestep, population, discount, 
           ...
           Testing_ab_sc, Testing_ab_sc_ycum,
           cost_ab_sc, cost_ab_sc_ycum, cost_ab_sc_cum, cost_ab_sc_disycum, cost_ab_sc_discum, 
           Testing_RNA_sc, Testing_RNA_sc_ycum,
           cost_RNA_sc, cost_RNA_sc_ycum, cost_RNA_sc_cum, cost_RNA_sc_disycum, cost_RNA_sc_discum, 
           Testing_POCT_sc, Testing_POCT_sc_ycum,
           cost_POCT_sc, cost_POCT_sc_ycum, cost_POCT_sc_cum, cost_POCT_sc_disycum, cost_POCT_sc_discum,
           Treatment_sc, Treatment_sc_ycum,
           cost_TreatDAA_sc, cost_TreatDAA_sc_ycum,  cost_TreatDAA_sc_cum,
           cost_TreatDAA_sc_disycum,cost_TreatDAA_sc_discum, 
...
x <- Resflow_sc_year_pop$dfList_NP_2023%>%group_by(population, year)

View(x)
x_num <-  Num_box$dfList_NP_2023%>%filter(cascade == "undiag")%>%
