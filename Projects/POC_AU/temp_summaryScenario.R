...
# replace calibrateInit = scenarios 
...
#### N: each subpop/ total  #### 
# subpop as list
Res <- function(pj,calibrateInit ,endY){ 
  subpop_N[[i]] <- lapply(POC_AU$popNames, function(x){ 
    
    a <- popResults_MidYear(POC_AU, Sce_epi[[i]],
                            Population = x,
                            Disease_prog = NULL, 
  ...
  total_N[[i]] <- pop_N[[i]]%>%group_by(year)%>%summarise(best = sum(best))
  ...
  Outcome <- list("popPrev" = tempPrev_subpop, 
                          "settingPrev" = tempPrev_setting, 
                          "popPrevRNA" = tempPrevRNA_subpop, 
                          "settingPrevRNA" = tempPrevRNA_setting, 
                          "popHCVinc" = HCVInc_subpop, 
                          "settingHCVinc" = HCVInfect_setting
  )
...
save(Outcome_npscale,
     file = file.path(OutputFolder ,
                      paste0(project_name,"outcomes_npscale" ,".rda")))




Outcome_np <- list("popPrev" = tempPrev_subpop, 
                   "settingPrev" = tempPrev_setting, 
                   "popPrevRNA" = tempPrevRNA_subpop, 
                   "settingPrevRNA" = tempPrevRNA_setting, 
                   "popHCVinc" = HCVInc_subpop, 
...
save(Outcome_np,
     file = file.path(OutputFolder ,
                      paste0(project_name,"outcomes_np" ,".rda")))
