# =============================================================================
# Results summary across 5 scenarios — v2
#
# CHANGES vs previous version
# ---------------------------------------------------------------------------
# 1. cost_types reduced from 3 to 2: c("fixednvariable", "total").
#    "DAAcost_reduchalf" is NO LONGER a cost type. DAA unit price is applied
#    post-hoc in Result_aggregate_sensitivity_v2.R (cost never feeds back into
#    model dynamics), so a halved-DAA run is just one point on the price sweep.
#    -> Step 2 drops from 15 iterations to 10.
#
# 2. epi_cost_type is now "fixednvariable". Epi outcomes are cost-independent;
#    the cost type only selects which Simulations_*.rda / param_sc_* file the
#    epi arrays are read from. Any available basis gives identical epi output.
#
# 3. Sample count is derived from the data rather than hard-coded, so par_col
#    cannot silently desync from the actual number of parameter sets.
#
# Everything else is unchanged — the per-scenario save/drop/recombine pattern
# and the chunked mclapply helpers were already doing the right thing.
# =============================================================================
...
# ---------------------------------------------------------------------------
# CHANGE 1 — cost bases that require their own simulation output.
# Only bases whose NON-DAA (diagnosis / compartment) costs differ belong here.
# ---------------------------------------------------------------------------
cost_types <- c("fixednvariable", "total")

# ---------------------------------------------------------------------------
# CHANGE 2 — epi is cost-independent; read it from whichever basis exists.
# ---------------------------------------------------------------------------
epi_cost_type <- "fixednvariable"

# ---------------------------------------------------------------------------
# CHANGE 3 — sample count derived, not assumed.
# ---------------------------------------------------------------------------
n_samples <- if (!is.null(POC_AU$numberSamples)) POC_AU$numberSamples else 1000L
par_col   <- c("best", paste0("set", seq_len(n_samples)))
cat(sprintf("Parameter sets: %d (par_col length %d)\n", n_samples, length(par_col)))
...
# =============================================================================
# Pre-run checks — fail fast rather than 40 minutes in
# =============================================================================
cat("\n=== Pre-run checks ===\n")
.sim_f <- file.path(RDAFolder, paste0(project_name, "Simulations_", epi_cost_type, ".rda"))
if (!file.exists(.sim_f)) stop("Missing epi simulation file: ", .sim_f)
for (ct in cost_types) {
  f <- file.path(RDAFolder, paste0(project_name, "Simulations_", ct, ".rda"))
  if (!file.exists(f)) stop("Missing: ", f)
}
cat("[ok] Simulations_*.rda present for: ", paste(cost_types, collapse = ", "), "\n\n")
...
# =============================================================================
cat(sprintf("\n=============================================================\n"))
cat(sprintf("STEP 1: Epi outcomes (read from cost basis: %s)\n", epi_cost_type))
cat(sprintf("=============================================================\n"))
...
for (scn in sce_name) {
  ...
  for (i in seq_along(par_Num_box)) {
    Num_box[[scn]][, paste0("set", i)] <- par_Num_box[[i]]$best
  }
  rm(par_Num_box); gc()
  
  Num_box[[scn]] <- Num_box[[scn]] %>%
    select(year, population, state, timestep, cascade, disease_prog, all_of(par_col))
  ...
  for (i in seq_along(par_Sce_flow)) {
    for (x in non_cost_flow_names) {
...
for (nm in epi_result_names) assign(nm, list())

for (scn in sce_name) {
  f <- epi_scn_file(scn)
  if (!file.exists(f)) stop("Missing scenario epi result file: ", f)
...
# (cost-basis independent — consumed once by Result_aggregate_sensitivity_v2.R)
# =============================================================================
Resflow_dt    <- list()
Resflow_sc_dt <- list()
for (scn in sce_name) {
...
# =============================================================================
# STEP 2 — COST & QALY, for the 2 cost bases only
#
# NOTE: cost_Treatment / cost_Retreat extracted here still carry whatever DAA
# price was baked into param_cost. That is FINE and deliberate — the aggregate
# script deletes both columns and rebuilds DAA cost from treatment VOLUMES.
# Nothing downstream depends on the embedded DAA price.
# =============================================================================
...
for (cost_type in cost_types) {
  cat(sprintf("\n=============================================================\n"))
  cat(sprintf("STEP 2: Cost & QALY for cost basis: %s\n", cost_type))
  cat(sprintf("=============================================================\n"))
  ...
  for (scn in sce_name) {
    ...
    for (i in seq_along(par_cost_box)) {
    ...
    for (i in seq_along(par_QALY_box)) {
    ...
    for (i in seq_along(par_Sce_flow_cost)) {
      for (x in cost_flow_names) {
        Sce_flow_cost[[x]][, paste0("set", i)] <- par_Sce_flow_cost[[i]][[x]]$best
      }
    }
    rm(par_Sce_flow_cost); gc()
...
cat(sprintf("  Step 2 (cost): %s over %d iterations\n",
            fmt_time(proc.time()["elapsed"] - t_cost_start), n_cost_iter))
