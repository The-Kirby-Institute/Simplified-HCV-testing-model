...
scale_CT_key <- function(dfList_CT, dfList_base, dfList_NP, pj,
                         S_Yint, S_Yend, key) {
  ...
  n_stages <- dim(dfList_base[[key]])[2]
  
  for (s in 1:n_stages) {
    for (pop in 1:5) {
      for (t in t_range) {
        
        base_ann <- dfList_base[[key]][pop, s, t]
        np_ann   <- dfList_NP[[key]][pop, s, t]
        
        if (pop %in% 1:2) {
          # Community — annual-scale collision check
          ct_val <- if (base_ann + np_ann <= 1) {
            base_ann
          } else {
            pmax(0, 1 - np_ann)
          }
          ...
          # Prison — monthly-scale collision check
          base_m <- annual_to_monthly(base_ann, pj)
          np_m   <- annual_to_monthly(np_ann,   pj)
          ct_m   <- if (base_m + np_m <= 1) {
            base_m
          } else {
            pmax(0, 1 - np_m)
          }
          ct_val <- monthly_to_annual(ct_m, pj)
        }
        
        dfList_CT[[key]][pop, s, t] <- ct_val
      }
    }
  }
...
scale_CT_eta <- function(dfList_CT, dfList_base, dfList_NP, pj, S_Yint, S_Yend) {
  scale_CT_key(dfList_CT, dfList_base, dfList_NP, pj, S_Yint, S_Yend, key = "eta")
}

scale_CT_RNA <- function(dfList_CT, dfList_base, dfList_NP, pj, S_Yint, S_Yend) {
  scale_CT_key(dfList_CT, dfList_base, dfList_NP, pj, S_Yint, S_Yend, key = "tau_RNA")
}

scale_CT_ab <- function(dfList_CT, dfList_base, dfList_NP, pj, S_Yint, S_Yend) {
  scale_CT_key(dfList_CT, dfList_base, dfList_NP, pj, S_Yint, S_Yend, key = "tau_ab")
}
