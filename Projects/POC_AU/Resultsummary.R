...
for(cost_type in cost_types){ 
  ...
  load(file.path(OutputFolder, paste0(project_name, "Simulations_", cost_type,".rda")))
  ...
  save(Num_box, pop_N, commu_N, prison_N, prisonPWID_N, 
       ...
       file = file.path(OutputFolder,paste0(project_name,"epiRes_timestep_sq_", cost_type ,".rda"))
  ...
  save(Num_box, Resflow_dt, Resflow_sc_dt, 
       ...
       file = file.path(OutputFolder,
                        paste0(project_name,"Res_dt_sq_",cost_type ,".rda"))
  )
  
  
  rm(Num_box, par_Num_box, param_sq, Sce_sq,
