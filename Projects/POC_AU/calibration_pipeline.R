...
#best_est_pop <- calibrateInit$allPops[,,1]
endY = 100
# =============================================================================
# 1. INPUTS
# =============================================================================
...
for (yr in cal_years) {
  ...
  dfList_CT_current <- scale_CT_RNA(dfList_CT_current, dfList, dfList_NP_current,
                                    POC_AU, yr, yr + 1)
  dfList_CT_current <- scale_CT_ab( dfList_CT_current, dfList, dfList_NP_current,
                                    POC_AU, yr, yr + 1)
...
# =============================================================================
# 11. SAVE CALIBRATED NP SCENARIO FOR 2025-2030 FORWARD WORK
# =============================================================================

# Convergence metadata for traceability
np_calibration_meta <- list(
  cal_years          = cal_years,
  converged          = all_converged,
  passes_run         = if (exists("pass")) pass else NA_integer_,
...
# Final fit vs obs (snapshot the diagnostic table for later reference)
np_fit_vs_obs <- local({
  rows <- list()
  for (yr in cal_years) {
    for (sett in c("Community", "Prison")) {
      pops <- if (sett == "Community") 1:2 else 3:5
      ab_mod  <- yr_sum_event(Sce_np_test$newTestingAb_sc,    POC_AU, pops, yr) +
        yr_sum_event(Sce_np_test$newTestingAb_sc_neg, POC_AU, pops, yr)
      rna_mod <- yr_sum_event(Sce_np_test$newTestingAg_sc,    POC_AU, pops, yr) +
        yr_sum_event(Sce_np_test$newTestingAg_sc_neg, POC_AU, pops, yr) +
        yr_sum_event(Sce_np_test$newTestingPOCT_sc,  POC_AU, pops, yr) +
        yr_sum_event(Sce_np_test$newTestingPOCT_sc_neg, POC_AU, pops, yr)
      tr_mod  <- yr_sum_event(Sce_np_test$newTreatment_sc, POC_AU, pops, yr)
      o <- obs_data |> dplyr::filter(year == yr, setting == sett)
      rows[[length(rows) + 1]] <- tibble(
        year = yr, setting = sett,
        Ab_mod = ab_mod,   Ab_obs  = o$T_ab,
        RNA_mod = rna_mod, RNA_obs = o$T_rna,
        Tr_mod = tr_mod,   Tr_obs  = o$Tr
      )
    }
  }
  bind_rows(rows)
...
# Save everything needed to extend into 2025-2030 scenarios
save(
  # --- Calibrated parameters (per-year, per-pop) ---
  Ccal_list,              # Cov_np list, named by year then pop
  cal_list,               # Full calibration objects: Cov_neg, fc, monthly, stocks
  fc_list,                # Per-year fc matrices
  fc_full,                # Combined fc matrix across all cal years
  
  # --- Built cascade arrays (extend these for 2025+) ---
  dfList_NP_2024,         # NP cascade parameters, ready to extend
  dfList_CT_NP_2024,      # CT cascade after displacement, ready to extend
  
  # --- Final converged scenario (use as starting point for 2025+) ---
  Sce_np_test,            # Full HCV_np output: allPops, newTreatment_sc, etc.
  
  # --- Inputs to reuse in scenario scripts ---
  NPlst,
  NP_eta_C, NP_eta_P,
  frac_test,
  obs_data,
  ...
  file = file.path(OutputFolder, paste0(project_name, "_NPcal_2024.rda"))
)

cat(sprintf("\nSaved calibrated NP scenario to: %s\n",
            file.path(OutputFolder, paste0(project_name, "_NPcal_2024.rda"))))
print(np_fit_vs_obs)
